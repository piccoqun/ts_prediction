...
from hyperparameter_search.gs_manager import GSManager
from hyperparameter_search.reinforce import Reinforce
import datetime


'''
max_layer in NAS --> max nodes in the workflow
num_of_hyperparameters --> number of hyperparameters for each node
'''
def parse_args():
...
def train():
    ...
    gs_manager = GSManager(num_of_hyperparameters,
    ...
    for i_episode in range(MAX_EPISODES):
        ...
        if all(ai > min_action for ai in action[0][0]) and all(ai < max_action for ai in action[0][0]):
            reward, pre_acc = gs_manager.get_reward(action, step, pre_acc)
